...
test("the runner pins every qualified ACPX production dependency", () => {
  assert.equal(runnerPackage.dependencies["@openai/codex"], "0.154.0");
  ...
  assert.equal(rootPackage.pnpm.overrides["@agentclientprotocol/claude-agent-acp@0.70.0>@anthropic-ai/claude-agent-sdk"], "0.3.267");
...
test("old and new pnpm configuration both apply the exact runtime patches", () => {
  ...
  assert.match(codexPatch, /\+    "@openai\/codex": "0\.154\.0"/);
});
