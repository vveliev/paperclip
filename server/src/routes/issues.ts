...
const ISSUE_WAKE_DIAGNOSTIC_KNOWN_REASONS = new Set([
  ...
  "status_card_compile_assigned",
  "status_card_update_assigned",
  "secret_proposal_resolved",
  "summary_slot_generation_requested",
...
export function issueRoutes(
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      // comment will actually move it out of that terminal status -- that's
      // the only path below that can dispatch a run against the workspace
      ...
      // blockers) leaves the issue terminal and never touches the workspace,
      // so forcing a rebuild here just turns an unrelated git/provisioning
      // failure into a 503 that blocks posting the comment at all (BLA-1027).
