...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
  ...
  async function reconcileStrandedAssignedIssues(opts?: { issueCreatedAtGte?: Date | null }) {
    ...
    for (const issue of candidates) {
     try {
      ...
      // skipping every later issue, plus issue-graph-liveness, task-watchdog,
      // silent-active-run, stale-lock, and productivity-review reconciliation
      // for that whole tick — this is what happened in production on
      // 2026-09-03.
      logger.error(
  ...
  async function ensureIssueBlockedByEscalation(input: {
    ...
    const updated = await updateIssueTolerateTerminalConflict(input.issue.id, update, "ensureIssueBlockedByEscalation");
    if (!updated) return null;
