  ...
  if (heartbeat) {
    ...
    if (heartbeatSchedulingSuppression.suppressed) {
      ...
      const startupHeartbeatRecovery = (async () => {
        ...
        // watchdogs, silent-active-run scanning, and stale-lock sweeping
        // included. This is the same class of bug the periodic tick chain
        // had; see the equivalent per-stage split further down.
        try {
    ...
    startHeartbeatSchedulerInterval(() => {
      ...
      trackHeartbeatSchedulerWork((async () => {
        ...
        if (!(await heartbeat.resolveSchedulingSuppression()).suppressed) {
          ...
          // of the tick — dependency-wake reconciliation, task watchdogs,
          // silent-active-run scanning, and stale-lock sweeping included — this
          // is what happened in production on 2026-09-03. A stage failing now
          // is logged and the sweep still moves on.
          trackHeartbeatSchedulerWork((async () => {
            try {
